...
<div style="max-width:760px;margin:0 auto;padding:24px 16px;">
    ...
    <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:20px;flex-wrap:wrap;gap:12px;">
        <div class="sort-tabs">
            <a href="{{ route('home', ['sort'=>'latest']) }}"  class="sort-tab {{ $sort==='latest'  ? 'active':'' }}">🕐 New</a>
            <a href="{{ route('home', ['sort'=>'top']) }}"     class="sort-tab {{ $sort==='top'     ? 'active':'' }}">🔥 Top</a>
            <a href="{{ route('home', ['sort'=>'trending']) }}" class="sort-tab {{ $sort==='trending'? 'active':'' }}">📈 Trending</a>
        </div>
        @auth
    ...
    @if(session('success'))
        <div style="background:rgba(34,197,94,.1);border:1px solid rgba(34,197,94,.3);color:#86efac;padding:10px 16px;border-radius:8px;font-size:.85rem;margin-bottom:16px;">
            ✅ {{ session('success') }}
        </div>
    @endif
